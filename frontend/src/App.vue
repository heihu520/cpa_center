...
const activeConnectionId = computed(() => settingsStore.activeConnectionId)
const connectionOptions = computed(() => settingsStore.connections)
...
const shellClasses = computed(() => ({
  ...
  'app-shell--mobile': shellMode.value === 'mobile',
}))
...
async function calibrateWindowToScreen() {
  try {
    ...
    const screen = screens.find((item: { isCurrent: boolean }) => item.isCurrent)
      ?? screens.find((item: { isPrimary: boolean }) => item.isPrimary)
      ?? screens[0]
    if (!screen) {
      return
    }
        ...
        <main class="app-main">
          ...
          <header class="topbar">
            ...
            <div class="topbar-meta">
              ...
              <el-select
                v-if="connectionOptions.length > 0"
                class="locale-switcher"
                :model-value="activeConnectionId"
                size="small"
                @change="changeActiveConnection"
              >
                <el-option
