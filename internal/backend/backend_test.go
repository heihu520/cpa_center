...
func TestBackendTestAndSaveSettingsSyncsInventory(t *testing.T) {
	...
	if configHits != 0 {
		t.Fatalf("expected no config fetch during test-and-save, got %d", configHits)
	}
	if fetches != 1 {
		t.Fatalf("expected one auth-files fetch during test-and-save, got %d", fetches)
	}
